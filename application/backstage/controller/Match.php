...
//        }
    ...
    public function finishRound($area_id = '', $age = '')
    {
        ...
        $list = (new RaceRecord())
            ->field('uid,round,name,sex,age,sort,race_id,area_id,create_time')
            ->where([
            'area_id' => $area_id,
            'age' => ['between', $age],
            'round' => $round,
     ...
     * @throws
     */
    private function finishField($area_id = '')
    {
        if (empty($area_id))
...
//        ])->find();
//
//        if ($res) $this->error('比赛还未完成,请勿结束比赛');
