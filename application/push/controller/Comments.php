...
class Comments extends BBComments
{
    ...
    public function get_comments_list()
    {
        ...
        $push_id = input('?param.id')?(int)input('param.id'):0;
        $start_id = input('?param.startid')?(int)input('param.startid'):0;
        $length = input('?param.length')?(int)input('param.length'):20;
        $ListDB = self::Get_comments('bb_rewind_comments',$push_id,$start_id,$length,'bb_rewind_comments_like',$uid);
        if (count($ListDB) == $length)
        {
            return ['data'=>$ListDB,'comments_count'=>self::Get_comments_count('bb_rewind_comments',$push_id),'is_bottom'=>0,'code'=>1];
        }
        else
        {
            return ['data'=>$ListDB,'comments_count'=>self::Get_comments_count('bb_rewind_comments',$push_id),'is_bottom'=>1,'code'=>1];
        }
    }
    //获得回复评论列表
    public function get_comments_reply_list()
    {
        ...
        $start_id = input('?param.startid')?(int)input('param.startid'):0;
        $length = input('?param.length')?(int)input('param.length'):20;
        $ListDB = self::Get_reply_comments('bb_rewind_comments_reply',$comments_id,$start_id,$length,'bb_rewind_comments_like',$uid);
        if (count($ListDB) == $length)
        {
    ...
    public function comments()
    {
        ...
        return ['code'=>self::Send_comments('bb_rewind_comments',$uid,$record_id,$content,$score)];
    }
    //回复评论
    public function reply()
    {
        ...
        $comments_id = input('?param.comments_id')?(int)input('param.comments_id'):0;
        $content = input('?param.content')?(string)input('param.content'):'';
        return ['code'=>self::Send_reply_comments('bb_rewind_comments_reply',$uid,$comments_id,$content)];
    }
    //评论点赞
    public function comments_like()
    {
        ...
        return self::_like('bb_rewind_comments_like',$id,$uid,1);
    }
    //取消评论点赞
    public function comments_un_like()
    {
        $uid = input('?param.uid')?(int)input('param.uid'):0;
        $id = input('?param.id')?(int)input('param.id'):0;
        return self::_un_like('bb_rewind_comments_like',$id,$uid,1);
    }
    //回复点赞
    public function reply_like()
    {
        $uid = input('?param.uid')?(int)input('param.uid'):0;
        $id = input('?param.id')?(int)input('param.id'):0;
        return self::_like('bb_rewind_comments_like',$id,$uid,2);
    }
    //取消回复点赞
    public function reply_un_like()
    {
        $uid = input('?param.uid')?(int)input('param.uid'):0;
        $id = input('?param.id')?(int)input('param.id'):0;
        return self::_un_like('bb_rewind_comments_like',$id,$uid,2);
    }
}