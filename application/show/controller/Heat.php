...
class Heat 
{
    ...
    public  function movies_v2($uid=0,$startid=0,$length = 4,$type=1)
    {
        if (\app\user\model\Exists::userhExists($uid)!=1) {
            return ['code'=>0, 'message'=>'用户不存在' ];
        }
        ...
        foreach ($DBList as $DB)
        {
            ...
            $temp->self_uid = $uid;
            $Data[]= $temp->get_all_subject_redis();
        ...
        return ['code'=>1, 'data'=>[ 'list' =>$Data,'is_bottom'=>$is_bottom ,   ]    ];
    }
