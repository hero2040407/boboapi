 ...
 * User: 谢烨
 */
class Dasai extends Ach implements UpdateInterface {
    ...
    public function update_ach($param) {
        ...
        $old_ach = $this->ach->dasai;
        
        // 不管怎样，需要更新汇总表
        $ach_obj = $this->ach;
        $summary_obj = $this->ach_summary;
        $summary_obj->dasai += $param;
        $summary_obj->save ();
        
        // 根据现在的level，来判断是否给成就表更新数据。
        $new_ach = self::jisuan ( $summary_obj->dasai );
        
        if ($old_ach != $new_ach) {
            $ach_obj->dasai = $new_ach;
            $ach_obj->save ();
