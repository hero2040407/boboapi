...
class ExpDashang extends ExpInterface
{
    ...
    public function add_exp(Exp $exp)
    {
       ...
       $present_id = intval( $exp->present_id);
       
       // $limit  = self::EXP_LIMIT;// 用户的当日直播经验上限。
        
        $db = Sys::get_container_db();
        $sql ="select experience from bb_present where id = {$present_id}";
        $temp = $db->fetchOne($sql);
        $temp = intval($temp);
